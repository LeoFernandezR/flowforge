...
```bash
git add prisma/schema.prisma prisma/migrations src/lib/flow/runFlow.ts src/lib/flow/runFlow.test.ts
git commit -m "feat: persist batchId on Run"
```

> Note: `src/generated/prisma` is gitignored (`.gitignore`) and regenerated by `prisma generate` /
> the migration — do **not** add it to the commit. The migration under `prisma/migrations` is
> tracked; the generated client is not.
